...
namespace ElementsAndMaterials
{
    public partial class Form1 : Form
    {
        ...
        private void makeRequestToShowOne(string nameElement) => selectSearchType(textBox2.Text);
        ...
        private void makeRequestToInsertOne(string identificatorElement)
        {
        ...
        private void selectSearchType(string identificatorElement)
        {
            switch (comboBox2.SelectedIndex)
            {
                case 0:
                    SQLhandler.makeRequestShowElements(com, con, "", "", textBox1, "select_1_element_witch_no_in_materials_by_name.sql", identificatorElement);
                    break;
                case 1:
                    if (identificatorElement.All(char.IsDigit))
                        SQLhandler.makeRequestShowElements(com, con, "", "", textBox1, "select_1_element_witch_no_in_materials_by_ID.sql", identificatorElement);
                    else
                        textBox1.Text = " НЕПРАВИЛЬНО ВЫБРАН ТИП ПОИСКА ";
                    break;
                default:
